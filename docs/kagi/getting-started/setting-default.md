...
2. Right-click the Chrome address bar and choose **Manage Search Engines and Site Search**
3. In the **Site Search** section (about halfway down), click the **Add** button (upper right of Site Search section) and fill in these details:
    - **Name:** Kagi Search
    - **Shortcut:** kagi
    - **URL with %s in place of query:** `https://kagi.com/search?q=%s` if you're not planning to use Kagi in private search. If you are, then use `https://kagi.com/search?token=TOKEN&q=%s` (replace TOKEN with the token found in your [private session link​](#private_session))
4. Find Kagi again in the Site Search section. You may have to click **Additional sites** at the bottom if you don't see it right away.
5. Click the three dots next to the Kagi entry in the list.
